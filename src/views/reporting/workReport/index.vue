  ...
  <div>
    ...
    <div class="workList">
      ...
      >
        ...
        <el-table-column label="汇报时间">
          <template #default="scope">
            {{ formatTime(scope.row.reportCycle) }}
          </template>
        </el-table-column>
      ...
      <pagination
        ...
        @pagination="findAll"
      />
      <!-- 新建汇报抽屉  submitSuccess 监听新建汇报提交成功事件-->
      <WorkDrawer
...
const openReportDetails = async (scoped: any) => {
  ...
  try {
    // 调用接口获取详情数据
    const response = await WorkReportsAPI.details(reportId);
...
const upd = async (scoped: any) => {
  const Id = scoped.id;
  // 调用接口获取详情数据
  const response = await WorkReportsAPI.details(Id);
...
const handleAll = () => {
  ...
  pageQuery.keywords = "";
  pageQuery.deptId = "";
  pageQuery.reportCycle = "";
  findAll();
};

// 获取表格数据
const findAll = () => {
  WorkReportsAPI.workReportsPage(pageQuery).then((res: any) => {
