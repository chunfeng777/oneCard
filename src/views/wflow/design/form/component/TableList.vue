  ...
  >
    ...
    >
      ...
      <el-table-column
        ...
        :prop="col.key"
        :label="col.name"
        v-for="(col, i) in config.props.columns"
        :key="`col_${col.id}`"
        :class-name="`${mode === 'free' ? 'w-form-d-item' : ''} ${
          _active?.id === col.id && freeMode ? 'w-form-cp-active' : ''
        }`"
      >
        <template #header>
          <div
            style="border: none !important"
            :class="{ 'w-form-cp-active': _active?.id === col.id && freeMode }"
          >
        ...
        <template v-slot="scope">
          <form-component
            ...
            v-model="_value[scope.$index][col.key]"
            :key="col.id"
            :type="col.type"
          />
        </template>
