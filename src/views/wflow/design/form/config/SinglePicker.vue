...
function addOption() {
  props.config.props.options.push(
    `选项${props.config.props.options.length + 1}`
  );
}
</script>
...
<template>
  ...
  <el-form-item label="选项设置">
    ...
    <vue-draggable
      ...
      handle=".w-move"
      :animation="150"
      v-model="config.props.options"
    >
      <el-input
        v-for="(option, i) in config.props.options"
        v-model="config.props.options[i]"
        :key="i"
      >
        <template #prepend>
