...
import { reloadNodeId } from "@/utils/ProcessUtil.ts";
import nodeMixin from "../NodeMixin.ts";
import nodeType, { NodeComponents } from "../ProcessNodes.ts";
...
function addBranch() {
  ...
  if (nodeType[type]) {
    props.modelValue.props.branch.splice(
      index,
      0,
      nodeType[type].createSelf(index + 1)
    );
...
function select(nd, i) {
  if (
    !(
      i === props.modelValue.branch.length - 1 &&
      ...
      >
        <div>
          ...
          <component
            ...
            :moveRn="
              i <
              modelValue.branch.length -
                (modelValue.props.type !== 'Parallel' ? 2 : 1)
            "
            :moveLn="i > 0"
            :is="NodeComponents[modelValue.props.type]"
            :isDefault="
              i === modelValue.branch.length - 1 &&
              modelValue.props.type !== 'Parallel'
            "
            @insert-node="(type) => insertNode(nodes, -1, type)"
          ...
          />
          <!--渲染分支-->
          <template v-for="(item, bi) in nodes" :key="bi + '_bi'">
            <!--分支里面继续遍历-->
            <component
              ref="node"
              :readonly="readonly"
              v-model="modelValue.branch[i][bi]"
              :is="NodeComponents[item.type]"
              :branch="nodes"
              :index="bi"
