 ...
 * @param json 表单的json
 */
export const formInit = (ref: any, json: any) => {
  if (ref) {
 ...
 */
const loadFormItem = (item: any, items: any, preName: any, addItemFunc: any) => {
  formItemMap.set(item.id, item)
  if (Array.isArray(item)) {
    item.forEach(it => loadFormItem(it, items, null, addItemFunc))
  } else if (item.props.isContainer) {
    item.props.columns.forEach((it: any) => loadFormItem(it, items, null, addItemFunc))
  } else if (item.type === 'TableList') {
    //处理表格
    addItemFunc(item, preName)
    item.props.columns.forEach((col: any) => loadFormItem(col, items, item.name, addItemFunc))
  } else {
    addItemFunc(item, preName)
  }
}
 ...
 * @returns {*[]} 表单组件列表
 */
export const resolveFormJson = (json: any) => {
  let items: any[] = []
  formItemMap.clear()
  loadFormItem(json, items, null, (item: any) => items.push(item))
  return items
}
...
export const getFormPermFields = (defaultPerm = 'R') => {
  ...
  const addItem = (item: any, preName: any) => {
    items.push({
      id: item.id,
      key: item.key,
 ...
 */
export function showFields(fieldIds: any) {
 ...
 * @param fieldIds 字段id列表
 */
export function hideFields(fieldIds: any) {
 ...
 * @param fieldIds 字段id列表
 */
export function disableFields(fieldIds: any) {
 ...
 * @param fieldIds 字段id列表
 */
export function enableFields(fieldIds: any) {
