<template>
  <div class="myApplication">
    ...
    >
      ...
      <!-- <el-table-column prop="state" label="摘要信息" /> -->
      ...
      <el-table-column prop="endTime" label="结束时间">
        <template #default="scope">
          <span>{{ formatEndTime(scope.row.endTime) }}</span>
        </template>
      </el-table-column>
      ...
      <!-- <el-table-column label="操作">
        <template #default="scope">
          <el-button
            type="primary"
            link
            @click="(e) => handleRowDbClick(scope.row, e)"
            >再次提交</el-button
          >
        </template>
      </el-table-column> -->
...
const postApplyList = () => {
  OrgAPI.postApplyList(pageQuery).then((res) => {
    // 清空现有数组
    tableData.length = 0;
    // 使用 push 添加新数据
