...
// 查询所有文件列表
const getStatementsFiles = () => {
  StatementsAPI.getStatementsFiles(pageQuery).then((res) => {
    // 清空现有数组
    tableData.length = 0;
    // 使用 push 添加新数据
...
const customRequest = async (options) => {
  ...
  try {
    ...
    // 调用接口发送文件
    await StatementsAPI.fileOss(formData); // 直接调用 fileOss 上传文件
...
const handleDelete = async (row) => {
  ...
  await StatementsAPI.statementsFilesDelete(row.id);
